...
md %temp%\AutoMenus
for %%a in (5.2 5.3) do call :build %%a
...
call "%epic%\Engine\Build\BatchFiles\RunUAT.bat" BuildPlugin -Plugin="%CD%\AutoMenus.uplugin" -Package="%temp%\AutoMenus\%1" -Rocket
...
del AutoMenus-%1.zip
tar -a -c -f AutoMenus-%1.zip -C %temp%\AutoMenus\%1\ *
goto :eof
